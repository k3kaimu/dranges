...
/**
Count Types that the pred is true.
*/ 
template staticCount(alias pred, Types...)
{
    static if(Types.length){
        static if(pred!(Types[0]))
            enum staticCount = 1 + staticCount!(pred, Types[1..$]);
        else
            enum staticCount = staticCount!(pred, Types[1..$]);
    }else
        enum staticCount = 0;
}

...
----
*/
auto nmap(alias fun, R)(R range) if (isForwardRange!R)
{
    auto s = segment!(dranges.functional.arity!fun)(range);
    alias Prepare!(fun, TypeNuple!(ElementType!R, dranges.functional.arity!fun)) pfun;
    return map!pfun(s);
}

template NMapType(alias fun, R)
{
    alias typeof(nmap!fun(R.init)) NMapType;
    //alias typeof(map!(Prepare!(fun, TypeNuple!(ElementType!R, dranges.functional.arity!fun)))(Knit!(TypeNuple!(R, dranges.functional.arity!fun)).init)) NMapType;
}
...
template tmap(alias fun, R...)
{
    auto tmap(R ranges)
    {
        static if(R.length == 1)
            return map!nf(ranges[0]);
        else
            return map!nf(knit(ranges));
    }
...
auto nfilter(alias fun, size_t step =1, R)(R range)
if (isForwardRange!R)
{
    alias Prepare!(fun, TypeNuple!(ElementType!R, dranges.functional.arity!fun)) pfun;
    return filter!pfun(stride(segment!(dranges.functional.arity!fun)(range), step));
}

template NFilterType(alias fun, size_t step, R)
if(__traits(compiles, nfilter!(fun, step)(R.init)))
{
    alias typeof(nfilter!(fun, step)(R.init)) NFilterType;
}
...
assert(equal(tf7, [tuple(0, 3.0, 2), tuple(2, 5.0, 2)]));
assert(equal(tf7.retro, [tuple(2, 5.0, 2), tuple(0, 3.0, 2)]));
...
assert(equal(tf9, [tuple(r1, 3.0, 2), tuple(r1, 4.0, 2)]));
----
*/
template tfilter(alias fun, alias select = [], bool normalElementReturn = false){
    
    auto tfilter(T...)(T args)if(T.length > 1 || (T.length == 1 && !is(ElementType!(T[0]).Types)))
    {
        return TFilter!(staticMap!(Unqual, T))(args);
    }
    
    auto tfilter(T...)(T args)if(is(ElementType!T.Types))
    {
        return filter!(Prepare!(fun, ElementType!T.Types))(args);
    }
    ...
    private:
        ...
        static if(select == []){
            ...
            template IndexOfRangeTypeTFImpl(size_t idx, Result...){
                static if(idx == select.length)
                    alias TypeTuple!(Result, TypeNuple!(false, T.length - Result.length)) IndexOfRangeTypeTFImpl;
                else{
                    static if(select[idx] == Result.length)
                        alias IndexOfRangeTypeTFImpl!(idx+1, Result, true) IndexOfRangeTypeTFImpl;
                    else static if(select[idx] > Result.length)
                        alias IndexOfRangeTypeTFImpl!(idx, Result, false) IndexOfRangeTypeTFImpl;
                    else
                        static assert(0, "tfilter Error : select array is invalid. select array must be sorted.");
                }
        ...
        static if(!normalElementReturn){
            template RangesTypesIndexImpl(size_t n){
                static if(n < T.length){
                    static if(IndexOfRangeTypeTF[n])
                        alias TypeTuple!(n, RangesTypesIndexImpl!(n+1)) RangesTypesIndexImpl;
                    else
                        alias RangesTypesIndexImpl!(n+1) RangesTypesIndexImpl;
                }else
                    alias TypeTuple!() RangesTypeIndexImpl;
            }
            
            static if(RangesTypesIndexImpl!(0).length > RangesTypes.length)
                alias RangesTypesIndexImpl!(0)[0..RangesTypes.length] RangesTypesIndex;
            else
                alias RangesTypesIndexImpl!(0) RangesTypesIndex;
        }
    ...
    public:
        this(T input){
            ...
            while(!this.empty && !_fun(_front().field)){
                foreach(i, isRange; IndexOfRangeTypeTF)
                    static if(isRange)
                        _input[i].popFront();
            }
            ...
            static if(allSatisfy!(isBidirectionalRange, RangesTypes) && allSatisfy!(hasLength, RangesTypes))
            {
                ...
                while(!this.empty && !_fun(_back().field)){
                    foreach(i, isRange; IndexOfRangeTypeTF)
                        static if(isRange)
                            _input[i].popBack();
                }
...
if(__traits(compiles, tfilter!fun(R.init)))
{
    alias typeof(tfilter!fun(R.init)) TFilterType;
}
...
----
*/
struct Scan(alias fun, S, R) if (/*isForwardRange*/isInputRange!R)// && CompatibilityFuncArgs!(fun, S, R))
{
    S _result;
    R _range;
    bool done;
...
/// ditto
Scan!(fun, S, R) scan(alias fun, S, R)(S seed, R range) if (/*isForwardRange*/isInputRange!R)// && CompatibilityFuncArgs!(fun, S, R))
{
    return Scan!(fun, S, R)(seed, range);
}

/// ditto
Scan!(fun, ElementType!R, R) scan(alias fun, R)(R range) if (/*isForwardRange*/isInputRange!R)// && CompatibilityFuncArgs!(fun, R))
{
    ElementType!R seed;
    if (!range.empty)
    {
...
auto r1 = [1,2,3,4];
auto f = flatMap!"std.range.repeat(a,a)"(r1); // stretching a range
...
----
*/
Concat!(typeof(map!fun(R.init))) flatMap(alias fun, R)(R range) {
    return concat(map!fun(range));
}
...
assert(equal(take(GRCF, 30), std.range.repeat(1, 30))); // Check
----
...
unittest
{
    ...
    // And now with BigInts!
    //auto fibs2 = unfold!((a,b) => tuple(a,b,a+b))(BigInt(0), BigInt(1));
...
----
*/
Without!(typeof(chain(R.init)), IntersectionType!R) symmetricDifference(R...)(R ranges) {
...
*/
auto pComp(alias gen, alias pred, R...)(R inputs)
if (allSatisfy!(isForwardRange, R))
{
...
template PCompType(alias gen, alias pred, R...) {
    alias typeof(pComp!(gen, pred)(R.init)) PCompType;
...
----
*/
auto runLengthEncode(R)(R range) {
...
template IndicesType(alias value, R) {
    alias PCompType!("a","b",Numbers, typeof(map!(equals!(value, ElementType!R))(R.init))) IndicesType;
}
...
----
*/
PCompType!("a","b", Numbers, typeof(map!pred(R.init))) positions(alias pred, R)(R range) if (isForwardRange!R) {
...
auto r = std.range.repeat(5,5); // 5,5,5,5,5
...
----
*/
Tuple!(typeof(filter!pred(R.init)), typeof(filter!(Not!pred)(R.init))) separate(alias pred, R)(R range) if (isForwardRange!R)
{
...
----
*/
Tuple!(typeof(takeWhile!pred(R.init)), R) span(alias pred, R)(R range)
{
...
struct Combinations(R...) if (allSatisfy!(isForwardRange, R)) {
    ...
    @property Combinations save() { return this;}
...
----
*/
ChooseType!(repetition, R) choose(uint repetition = withoutRepetition, R)(R range, size_t n) if (isForwardRange!R && !isInfinite!R)
{
...
unittest
{
    ...
    auto c = choose(cards, 5); // default is withoutRepetition
    assert(walkLength(c) == 52*51*50*49*48/(5*4*3*2)); // 2,598,960

    // A DNA chain is made of one of four bases (here denoted as A, C, G or T)
    // A codon is a 3-bases encoding for an amino-acid. How many codon are there?
    auto DNA = cast(char[])"ACGT";
    auto codons = choose!withRepetition(DNA, 3);   // 4*4*4 == 64 elements
    assert(walkLength(codons) == 64);
}
...
template NReduce(alias fun)
{
    ...
    {
        S result = seed;
        auto s = stride(segment!(dranges.functional.arity!fun-1)(range), dranges.functional.arity!fun-1);
        foreach(elem; s)
        {

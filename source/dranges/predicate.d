...
*/
/*
bool some(alias pred, R)(R range) if (isForwardRange!R && dranges.functional.arity!pred == 1){
...
bool some(alias pred, R)(R range) if (isForwardRange!R && dranges.functional.arity!pred > 1){
    auto sr = segment!(dranges.functional.arity!pred)(range);
    alias Prepare!(pred, TypeNuple!(Unqual!(ElementType!R), dranges.functional.arity!pred)) predicate;
...
bool some(alias pred, R)(R range) if (isForwardRange!R && dranges.functional.arity!pred == 0){
    return naryFun!pred();
}
*/
template some(alias pred, R)if(isInputRange!R){
        template ParameterGenerator(size_t N){
            alias TypeNuple!(Unqual!(ElementType!R), N) ParameterGenerator;
        }

        alias templateFunctionAnalysis!(naryFun!pred, ParameterGenerator, 0, 26) Result;


        bool some(R range){
            static if(Result.arity == 0){
                foreach(e; range)
                    if(naryFun!pred())
                        return true;
                return false;
            }else static if(Result.arity == 1){
                foreach(e; range)
                    if(naryFun!pred(e))
                        return true;
                return false;
            }else{
                auto sr = segment!(Result.arity)(range);
                foreach(e; sr)
                    if(naryFun!pred(e.field))
                        return true;
                return false;
            }
        }
...
----
*/
bool all(alias pred, R)(R range) if (isForwardRange!R && dranges.functional.arity!pred == 1) {
    alias unaryFun!pred predicate;
...
bool all(alias pred, R)(R range) if (isForwardRange!R && dranges.functional.arity!pred == 0) {
    return naryFun!pred();
}
...
*/
/+
bool isOneOf(alias range)(T element)if(is(typeof(find(range, element)))){

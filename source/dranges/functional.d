...
assert(dranges.functional.arity!foo0 == 0);
assert(dranges.functional.arity!foo1 == 1);
assert(dranges.functional.arity!foo2 == 2);
----
...
----
T foo(T)(T t) { ...};
auto a = dranges.functional.arity!foo; // error !
auto b = dranges.functional.arity!(foo!int); // OK.
//
int foov(int a...) { return 0;}
assert(dranges.functional.arity!foov == 1);
----
...
template arity(alias templateFun, alias ParameterGenerator, size_t N = 0)
{
    static if(is(typeof(templateFun(ParameterGenerator!N.init))))
        enum arity = N;
    else static if(N < 30)
        enum arity = dranges.functional.arity!(templateFun, ParameterGenerator, N + 1);
    else
        static assert(0, "arity Error : " ~ pred.stringof);
}
...
unittest
{
    ...
    assert(dranges.functional.arity!foo1 == 1);
    assert(dranges.functional.arity!foo2 == 2);
    assert(!__traits(compiles, dranges.functional.arity!foo3)); // does not work on non-instantiated functions
    assert(dranges.functional.arity!(foo3!(int,double)) == 3);  // foo3!(int, double) is a 3-args function

    int foov(int a...) { return 0;}
    assert(dranges.functional.arity!foov == 1);
    
    assert(dranges.functional.arity!"1" == 0);
    assert(dranges.functional.arity!"a" == 1);
    assert(dranges.functional.arity!"b" == 2);
...
{
    enum size_t SumOfArity = zero + dranges.functional.arity!fun;
}
...
template juxtapose(Funs...)
{
    Tuple!(staticFilter!(isNotVoid, ReturnTypes!Funs)) juxtapose(ParameterTypeTuples!Funs params) {
        ...
        foreach(i, Fun; Funs) {
            ...
            enum lastParam = firstParam + dranges.functional.arity!(Fun);
...
template juxtapose2(F...)
{
    Tuple!(ReturnTypes!F) juxtapose2(Tuple!(ParameterTypeTuples!F) params) {
        ...
        foreach(i, Unused; F) {
            enum firstParam = arities[i];
            enum lastParam = firstParam + dranges.functional.arity!(F[i]);
...
unittest
{
    ...
    int[] f5(int[] i) { i.reverse(); return i; }
...
*/
DefaultValues!(fun, dranges.functional.arity!fun, D) withDefaultValues(alias fun, D...)(D defaults) if (D.length <= dranges.functional.arity!fun)
{
    DefaultValues!(fun, dranges.functional.arity!fun, D) def;
...
----
*/
auto arrayApply(alias fun, T : U[], U)(T args) if (CompatibilityFuncArgs!(fun, TypeNuple!(U, dranges.functional.arity!fun))) {
    TypeNuple!(U, dranges.functional.arity!fun) at;
...
template arrayify(alias fun, T) {
    alias TypeNuple!(T, dranges.functional.arity!fun) TN;
    alias fun!(TN) fun2;
...
unittest {
    ...
    assert(dranges.functional.arity!afoo == 1); // afoo is indeed a unary function now.
...
template PrepareImpl(alias fun, T...)
{
    ...
    static if(T.length){
        auto result(Tuple!T args){
            return nfun(args.field);
        }
    }else{
        auto result(){
            return nfun();
...
template PrepareImpl(alias fun, T...)
//if(!__traits(compiles, fun.stringof[0..8] == "__lambda") || fun.stringof[0..8] != "__lambda")
{
    /*
    static if(is(typeof(fun) : string))
        alias naryFun!(fun, T.length) nfun; // transforms string as n-args templated function.
    ...
    static if (__traits(compiles, nfun!T)){ // Can I instantiate nfun!T?
        ...
        alias nfun infun;    // It's a function, do nothing.
        alias ParameterTypeTuple!nfun A;
        
        static if(A.length){
            auto result(Tuple!(A) args){
                return infun(args.field);
            }
        }else{
            auto result(){
                return infun();
    ...
    */
    /*
    static if(is(typeof(fun) : string)){
        alias naryFun!fun nfun; // transforms string as n-args templated function.
        
        static assert(__traits(compiles, nfun(T.init)));
        
        auto result(Tuple!T args){
            return nfun(args.field);
        }
    }else{
        static if (__traits(compiles, fun!T)){ // Can I instantiate nfun!T?
            
            static if(T.length){
                auto result(Tuple!(T) args){
                    return fun(args.field);
                }
            }else{
                auto result(){
                    return fun!T();
            ...
            static assert(isSomeFunction!fun);
            alias ParameterTypeTuple!fun A;
    ...
    */
    static if(T.length > 0){
        auto result(Tuple!T args){
            return naryFun!fun(args.field);
        }
    }else{
        auto result(U...)(Tuple!U args){
            return naryFun!fun(args.field);
        }
    }
}
...
if(__traits(compiles, fun.stringof[0..8] == "__lambda") && fun.stringof[0..8] == "__lambda")
{
    static assert(__traits(compiles, fun(T.init)));
    auto result(Tuple!T args){
        return fun(args.tupleof);
    }
}*/
